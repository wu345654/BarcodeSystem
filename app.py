...
def export_delivery_order(delivery_order_id):
    ...
    try:
        ...
        if details:
            ...
            ws.cell(row=total_row, column=8, value=total_quantity)
        ...
        ws.cell(row=current_row + 2, column=1, value='')
        ws.cell(row=current_row + 2, column=2, value=notes[2])
        
        # 增加行高，确保文字全部显示
        ws.row_dimensions[current_row].height = 40
        ws.row_dimensions[current_row + 1].height = 40
        ws.row_dimensions[current_row + 2].height = 40
        ...
        ws.merge_cells(f'B{current_row + 1}:L{current_row + 1}')
        ws.merge_cells(f'B{current_row + 2}:L{current_row + 2}')
        ...
        ws.cell(row=current_row, column=9, value='打印员:')
        ws.cell(row=current_row, column=10, value=session.get('name', '未知') or '未知')
        ...
        ws.cell(row=current_row, column=4, value='')
        ws.cell(row=current_row, column=5, value='审核日期:')
        ws.cell(row=current_row, column=6, value='')
        ws.cell(row=current_row, column=7, value='')
        ws.cell(row=current_row, column=8, value='')
        ws.cell(row=current_row, column=9, value='打印日期:')
        ws.cell(row=current_row, column=10, value=datetime.now().strftime('%Y-%m-%d'))
        ws.cell(row=current_row, column=11, value='')
        ws.cell(row=current_row, column=12, value='')
